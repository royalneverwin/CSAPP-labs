...
#王新昊
#iaddq description:
#iaddq指令要在取指阶段读取icode:ifun<-M1[PC], rA:rB<-M1[PC+1], valC<-M8[PC+2]和
#valP<-PC+10。在译码阶段读取valB<-R[rB]。在执行阶段计算valE = valB + valC。在访存阶
#段不需要操作。在写回阶段R[rB]<-valE。在更新PC阶段将PC<-valP。
#jm description:
#jm指令要在取指阶段读取icode:ifun<-M1[PC], rA:rB<-M1[PC+1], valC<-M8[PC+2]和valP<-
#PC+10。在译码阶段读取valB<-R[rB]。在执行阶段计算valE = valB + valC。在访存阶段读取
#valM = M8[R[rb]+V]。在写回阶段不需要操作。在更新PC阶段将PC<-valM。
####################################################################
